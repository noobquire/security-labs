...
var bytes = text.Select(s => StringToBytes(s)).ToArray();
string key = "For who would bear the whips and scorns of time";
BreakCipher(bytes, key);

static byte[] XorBytes(byte[] text, byte[] key)
{
...
/// <summary>
/// Get percentage of letter, whitespace and punctuation characters in text.
/// </summary>
...
void BreakCipher(byte[][] cipherTextRows, string key)
{
    double bestFitness = 0;
    var bestText = new List<string>();

    for (int i = 0; i < cipherTextRows.Length; i++)
    {
        List<string> currentText = new List<string>();
        for (int j = 0; j < cipherTextRows.Length; j++)
        {
            int length = Math.Min(cipherTextRows[i].Length, cipherTextRows[j].Length);
            byte[] arr1, arr2;

            arr1 = cipherTextRows[i].Take(length).ToArray();
            arr2 = cipherTextRows[j].Take(length).ToArray();

            var xor = XorBytes(arr1, arr2);
        ...
        var fitness = GetTextFitness(GetTextFromRows(currentText, key));
    ...
    ShowResults(bestText, key);
}

static void ShowResults(IEnumerable<string> textResults, string key)
{
    foreach (var item in textResults)
    {
        var length = Math.Min(item.Length, key.Length);
        Console.Write(item.Substring(0, length) + "\t");
        Console.WriteLine(item.Substring(length));
    }
}
...
static string GetTextFromRows(List<string> rows, string key)
{
    StringBuilder str = new StringBuilder();
    foreach (var item in rows)
    {
        var length = Math.Min(key.Length, item.Length);
        str.Append(item.Substring(0, length));
    }
